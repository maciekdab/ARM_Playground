...
#include "stm32f4xx_hal.h"		/* Device specific include files */
#include "pff_stm32.h"

void dly_100us(void) {
	for(uint32_t i = 0; i < CPU_TICKS_100US; ++i) {
		asm("NOP");
	}
}
...
void xmit_spi(BYTE d) {
	HAL_SPI_Transmit(&hspi2, &d, sizeof(BYTE), HAL_MAX_DELAY);
}
//------------------------------------------------------------------------------
/** Receive a byte from the card */
BYTE rcv_spi (void) {
	BYTE spi_recv_data = 0;
	BYTE spi_send_dummy_data = 0xFF;

	HAL_SPI_TransmitReceive(&hspi2, &spi_send_dummy_data, &spi_recv_data, sizeof(BYTE), HAL_MAX_DELAY);

	return spi_recv_data;
}
//------------------------------------------------------------------------------
