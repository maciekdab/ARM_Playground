...
#include "stm32f3xx_hal.h"		/* Device specific include files */
#include "pff_stm32.h"
...
void xmit_spi(BYTE d) {
	HAL_SPI_Transmit(&hspi1, &d, sizeof(BYTE), HAL_MAX_DELAY);
}
//------------------------------------------------------------------------------
...
BYTE rcv_spi (void) {
	...
	HAL_SPI_TransmitReceive(&hspi1, &spi_send_dummy_data, &spi_recv_data, sizeof(BYTE), HAL_MAX_DELAY);
